...
def test_extract_required_capabilities_includes_maximal_matching_unknown() -> None:
    span = _span()
    ground = ir.GroundIR(
        span=span,
        problems=(
                ...
                finds=(
                    ir.KFindDecl(
                        ...
                        unknown_type=ast.UnknownTypeRef(
                            span=span, kind="MaximalMatching", args=("G",)
                        ),
                    ),
                ),
                constraints=(),
    ...
    assert "unknown.graph.maximal_matching.v1" in required


def test_extract_required_capabilities_includes_tree_graph_unknowns() -> None:
    span = _span()
    ground = ir.GroundIR(
        span=span,
        problems=(
            ir.GroundProblem(
                ...
                finds=(
                    ir.KFindDecl(
                        ...
                        name="T",
                        unknown_type=ast.UnknownTypeRef(
                            span=span, kind="SpanningTree", args=("G",)
                        ),
                    ),
                    ir.KFindDecl(
                        span=span,
                        name="F",
                        unknown_type=ast.UnknownTypeRef(span=span, kind="Forest", args=("G",)),
                    ),
                ),
                constraints=(),
                objectives=(),
    ...
    assert "unknown.graph.spanning_tree.v1" in required
