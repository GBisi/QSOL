...
- `generic_bqm.qsol.toml`: sample scenario config
...
uv run qsol targets check \
  ...
  --config examples/generic_bqm/generic_bqm.qsol.toml \
  --runtime local-dimod
```
...
- `scenarios.baseline.sets.Vars`: variable names used to build binary decision variables
- `scenarios.baseline.params.L[var]`: linear bias per variable
- `scenarios.baseline.params.Q[i][j]`: quadratic bias for pair `(i, j)`
- `scenarios.baseline.params.C`: constant objective offset
